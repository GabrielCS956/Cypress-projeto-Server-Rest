...
describe('Segurança - rotas protegidas', () => {
  ...
  before(() => {
    ...
    // cria via API e guarda credenciais em alias
    cy.createUserApi(email, senha, { administrador: "false" }).then(user => {
      cy.wrap(user).as('userCommon');
    });

    // cria sessão reutilizável (login programático)
    cy.get('@userCommon').then(u => {
      cy.session(['user', u.email], () => {
        cy.loginApi(u.email, u.password || u.password); // ajusta se body usar outra chave
        // visita para garantir que a app carregou com a sessão
...
/*
//Verificar que logout remove token e impede novas chamadas
describe('Logout e revogação de acesso', () => {
  it('SC001 -Depois do logout não deve haver chamadas autenticadas', () => {
    // cria e loga via API
    cy.createUserApi().then(u => {
      cy.loginApi(u.email, u.password).then(() => {
        // garante que está logado na UI
